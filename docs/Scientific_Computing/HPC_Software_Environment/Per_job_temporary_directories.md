...
the database onto the SSD,
    ...
    cp /opt/nesi/db/Kraken2/standard-2018-09 * $TMPDIR

To get Kraken2 to read the DB from the SSDs (and not from `/opt`),
change the `#!['bash']KRAKEN2_DEFAULT_DB` variable,

    export KRAKEN2_DEFAULT_DB=$TMPDIR

The variable `#!['bash']KRAKEN2_DEFAULT_DB` simply points to the
database and is found by `module show Kraken2`.
