...
{% include "partials/support_request.html" %} if you would like some assistance or are having any difficulties with this service.

!!! prerequisite "Prerequisite: Globus account"
...
!!! prerequisite "Prerequisite: Openstack Client"
    You need to have the Openstack client installed on your device (e.g. `apt install python3-openstackclient` or `pip3 install python-openstackclient`).
...
3. Run `openstack token issue` and then `openstack container list`: this lists all the buckets in the project and you can confirm that the bucket you want to share on Globus is listed.
...
2. You will need to authenticate with an identity from NeSI Keycloak. Click on `Use my user_id@iam.nesi.org.nz identity` text.
    ![Globus_RDC_Transfer](../../assets/images/Globus_RDC_Transfer.png)
...
4. To set up your credentials, please click `Continue`. You will be shown a Globus page requiring you to sign in to RDC.
    ![Globus RDC Credentials](../../assets/images/Globus_RDC_Credentials_Setup.png)
