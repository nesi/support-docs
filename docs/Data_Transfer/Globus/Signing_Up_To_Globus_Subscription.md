---
created_at: '2021-08-27T03:18:13Z'
tags:
    - file_transfer
title: Signing up to a Free Globus Subscription
---

A Globus subscription is sometimes necessary for certain tasks, such as [moving data between two computers/personal endpoints](./Data_Transfer_Between_Personal_Endpoints.md).

!!! tip
    The name of the Mahuika subscription is `New Zealand eScience Infrastructure Standard`

To sign up to a **free** Globus Subscription:

1. Click the Search Tab and click on `Find a Subscription`:
    ![Globus_Between_Computers_1.png](../../assets/images/Globus_Between_Computers_1.png)
2. In the empty field line, search for `New Zealand eScience Infrastructure Standard`
    ![Globus_Between_Computers_2.png](../../assets/images/Globus_Between_Computers_2.png)
3. On the `New Zealand eScience Infrastructure Standard` subscription page, click on `Join this Subscription`:
    ![Globus_Between_Computers_3.png](../../assets/images/Globus_Between_Computers_3.png)
4. Select an identity and provide your first name, last name, and organisation for the subscription.
    Note: you may be required to authenticate yourself.
    ![Globus_Between_Computers_4.png](../../assets/images/Globus_Between_Computers_4.png)
5. You will see a message saying that `Membership is Pending`. Our support team will submit your request and
   email you back once you have been accepted into the subscription.
    ![Globus_Between_Computers_5.png](../../assets/images/Globus_Between_Computers_5.png)
