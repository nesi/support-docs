...
Currently, Mahuika has Intel Broadwell and [AMD Milan CPUs](Milan_Compute_Nodes.md).
...
and different limits, for more information see [Mahuika_Slurm_Partitions](Mahuika_Slurm_Partitions.md).
...
Māui only has a [single partition to which NeSI users are permitted to submit work](Maui_Slurm_Partitions.md).
...
access to [Māui's ancillary nodes](Maui_Ancillary.md),
