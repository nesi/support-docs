...
| ```{% raw %}{% include "partials/app_header.html" %}{% endraw %}``` | Info block | At the top of documents about particular software (TODO: elaborate). Internally uses `safe_include` rather than `include` for its own sub-partials, so bad/missing module-list.json data for one app blanks just this block instead of showing a macro error for the whole page. |
